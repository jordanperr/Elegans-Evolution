...
void TSearch::SetPopulationSize(int NewSize)
{
  ...
  for (int i = 1; i <= NewSize; i++)
    RandomStates[i].SetRandomSeed(rs.UniformRandomInteger(1, pow(2,(8*sizeof(int))-1)));
}
...
void TSearch::UpdatePopulationStatistics(void)
{
	...
	// If the best performance has improved, update BestPerf and BestVector
	if (MaxPerf > BestPerf)
	{
		UpdateBestFlag = 1;
		BestPerf = MaxPerf;
...
ostream& operator<<(ostream& os, TSearch& s)
{
	// Write the vector size and population size
	os << s.vectorSize << " " << s.Population.Size() << endl;
	// Write the generation number and the maximum number of generations
	os << s.Gen << " " << s.MaxGens << endl;
	// Write the random state
	WriteRandomState(os);
	// Write the selection mode
	switch (s.SelectMode) {
		case FITNESS_PROPORTIONATE: os << 1 << " "; break;
		case RANK_BASED: os << 2 << " "; break;
		default: cerr << "Invalid selection mode" << endl; exit(0);
	}
	// Write the reproduction mode
	switch (s.RepMode) {
		case HILL_CLIMBING: os << 1 << " "; break;
		case GENETIC_ALGORITHM: os << 2 << " "; break;
		default: cerr << "Invalid reproduction mode" << endl; exit(0);
	}
	// Write the crossover mode
	switch (s.CrossMode) {
		case UNIFORM: os << 1 << endl; break;
		case TWO_POINT: os << 2 << endl; break;
		default: cerr << "Invalid crossover mode" << endl; exit(0);
	}
	// Write the search initialized and re-evaluation flags, and the checkpoint interval
	os << s.SearchInitialized << " " << s.ReEvalFlag << " " << s.CheckpointInt << endl;
	// Write the search constraint vector
	os << s.ConstraintVector << endl;
	// Write the mutation variance
	os << s.MutationVar << " ";
	// Write the crossover probability
	os << s.CrossProb << endl;
	// Write the crossover template
	for (int i = 1; i <= s.vectorSize; i++)
		os << s.crossTemplate[i] << " ";
	os << endl;
	// Write the elitist fraction
	os << s.EFraction << " ";
	// Write the max expected offspring
	os << s.MaxExpOffspring << endl;
	// Set the precision
	os << setprecision(32);
	// Write out the peformance and parameter vector of the best individual
	os << s.BestPerformance() << "    " << s.BestIndividual() << endl;
	// Write out the performance and parameter vector of each individual in the population
	for (int i = 1; i <= s.Population.Size(); i++)
		os << s.Performance(i) << "    " << s.Individual(i) << endl;
	// Return the ostream
	return os;
}

istream& operator>>(istream& is, TSearch& s)
{
	int i;
	double f;
	TVector<int> iv;
	...
	int vsize, psize;
	is >> vsize;
	s.SetVectorSize(vsize);
	iv.SetSize(vsize);
	is >> psize;
	s.SetPopulationSize(psize);
	// Read the generation number and the maximum number of generations
	is >> s.Gen;
	is >> i; s.SetMaxGenerations(i);
	// Read the random state
	ReadRandomState(is);
	// Read the selection mode
	is >> i;
	switch (i) {
		case 1: s.SetSelectionMode(FITNESS_PROPORTIONATE);break;
		case 2: s.SetSelectionMode(RANK_BASED);break;
		default: cerr << "Invalid selection mode" << endl; exit(0);
	}
	// Read the reproduction mode
	is >> i;
	switch (i) {
		case 1: s.SetReproductionMode(HILL_CLIMBING);break;
		case 2: s.SetReproductionMode(GENETIC_ALGORITHM);break;
		default: cerr << "Invalid reproduction mode" << endl; exit(0);
	}
	// Read the crossover mode
	is >> i;
	switch (i) {
		case 1: s.SetCrossoverMode(UNIFORM);break;
		case 2: s.SetCrossoverMode(TWO_POINT);break;
		default: cerr << "Invalid crossover mode" << endl; exit(0);
	}
	// Read the search initialized and re-evaluation flags, and the checkpoint frequency
	is >> s.SearchInitialized;
	is >> i; s.SetReEvaluationFlag(i);
	is >> i; s.SetCheckpointInterval(i);
	// Read the search constraint
	for (int i = 1; i <= vsize; i++)
		is >> iv[i];
	s.SetSearchConstraint(iv);
	// Read the mutation variance
	is >> f; s.SetMutationVariance(f);
	// Read the crossover probability
	is >> f; s.SetCrossoverProbability(f);
	// Read the crossover template
	for (int i = 1; i <= s.vectorSize; i++)
		is >> iv[i];
	s.SetCrossoverTemplate(iv);
	// Read the elitist fraciton
	is >> f; s.SetElitistFraction(f);
	// Read the max expected offspring
	is >> f; s.SetMaxExpectedOffspring(f);
	// Read the peformance and parameter vector of the best individual
	is >> s.BestPerf;
	for (int i = 1; i <= vsize; i++)
		is >> s.bestVector[i];
